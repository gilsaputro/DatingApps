...
func TestPartnerHandler_PassPartnerHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFunc: func() {
				m.EXPECT().PassPartner(partner.PartnerServiceRequest{
					UserID:     1,
					IsVerified: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler.PassPartnerHandler(w, r)
			result := w.Result()
			...
			if string(resBody) != tt.want.body {
				t.Fatalf("PassPartnerHandler body got =%s, want %s \n", string(resBody), tt.want.body)
			}

			if result.StatusCode != tt.want.code {
				t.Fatalf("PassPartnerHandler status code got =%d, want %d \n", result.StatusCode, tt.want.code)
			}
		})
	}
}
