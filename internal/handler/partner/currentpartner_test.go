...
func TestPartnerHandler_CurrentPartnerHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFunc: func() {
				m.EXPECT().GetCurrentPartner(partner.PartnerServiceRequest{
					UserID:     1,
					IsVerified: true,
			...
			want: want{
				code: 200,
				body: `{"data":{"id":1,"fullname":"full","status":"PENDING","is_verified":true,"created_date":""},"code":200,"message":"success"}`,
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler.CurrentPartnerHandler(w, r)
			result := w.Result()
			...
			if string(resBody) != tt.want.body {
				t.Fatalf("CurrentPartnerHandler body got =%s, want %s \n", string(resBody), tt.want.body)
			}

			if result.StatusCode != tt.want.code {
				t.Fatalf("CurrentPartnerHandler status code got =%d, want %d \n", result.StatusCode, tt.want.code)
			}
		})
	}
}
