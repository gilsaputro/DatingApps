...
func TestPartnerHandler_LikePartnerHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFunc: func() {
				m.EXPECT().LikePartner(partner.PartnerServiceRequest{
					UserID:     1,
					IsVerified: true,
				}).Return(nil)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler.LikePartnerHandler(w, r)
			result := w.Result()
			...
			if string(resBody) != tt.want.body {
				t.Fatalf("LikePartnerHandler body got =%s, want %s \n", string(resBody), tt.want.body)
			}

			if result.StatusCode != tt.want.code {
				t.Fatalf("LikePartnerHandler status code got =%d, want %d \n", result.StatusCode, tt.want.code)
			}
		})
	}
}
